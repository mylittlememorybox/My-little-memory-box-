...
export default function MemoryDetailsPage() {
  ...
  const goToBookPage = (nextPage: number) => {
    if (nextPage < 0 || nextPage > 13) return;
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Αποτυχία αποστολής.");
  ...
  if (status === "success") {
    return (
      <main style={page}>
        <BookLogo onClick={() => setStatus("idle")} />
    ...
    <main style={page}>
      <style>{`
        @keyframes pageFlip {
          ...
          50% { transform: rotateY(-16deg); opacity: .72; }
          100% { transform: rotateY(0deg); opacity: 1; }
        }
      `}</style>

      <BookLogo onClick={() => goToBookPage(0)} />

      <form onSubmit={submit} style={book}>
        <div
          style={{
            ...bookPaper,
        ...
        >
          ...
          {bookPage === 0 && (
            <>
              ...
              <Section emoji="" badge="Οι πρώτες στιγμές" title="Ήσουν μια σταλίτσα...">
                <Field label="Η πρώτη φορά που σε κράτησα αγκαλιά">
                  <textarea style={textarea} value={values.firstHold} onChange={onChange("firstHold")} />
                </Field>
                ...
                <TwoCols>
                  <Field label="Ζύγιζες μόνο">
                    <input style={input} value={values.weight} onChange={onChange("weight")} />
                  </Field>

                  <Field label="Είχες ύψος">
                    <input style={input} value={values.height} onChange={onChange("height")} />
                  </Field>
                </TwoCols>

                <Field label="Επιστρέψαμε από το μαιευτήριο">
                  <textarea style={textarea} value={values.fromHospital} onChange={onChange("fromHospital")} />
                </Field>

                <Field label="Οι πρώτες στιγμές σου στο σπίτι μας ήταν">
                  <textarea style={textarea} value={values.firstHomeMoments} onChange={onChange("firstHomeMoments")} />
                </Field>

                <Field label="Η πρώτη φορά που χαμογέλασες">
                  <textarea style={textarea} value={values.firstSmile} onChange={onChange("firstSmile")} />
                </Field>
          ...
          {bookPage === 1 && (
            <Section emoji="" badge="Οι άνθρωποί σου" title="Ο κόσμος σου">
              <Field label="Οι γονείς σου">
                <textarea style={textarea} value={values.parents} onChange={onChange("parents")} />
              </Field>
              <Field label="Οι θείοι και οι θείες">
                <textarea style={textarea} value={values.unclesAunts} onChange={onChange("unclesAunts")} />
              </Field>
              <Field label="Οι παππούδες και οι γιαγιάδες">
                <textarea style={textarea} value={values.grandparentsGodparents} onChange={onChange("grandparentsGodparents")} />
              </Field>
              <Field label="Οι νονοί/ες σου και οι φίλοι που έγιναν οικογένεια">
                <textarea style={textarea} value={values.chosenFamilyFriends} onChange={onChange("chosenFamilyFriends")} />
              </Field>
              <Field label="Οι αξίες που θέλουμε να σου δώσουμε σαν γονείς">
                <textarea style={textarea} value={values.familyValues} onChange={onChange("familyValues")} />
              </Field>
            </Section>
          )}
          ...
          {bookPage === 2 && (
            <Section emoji="✨" badge="Μικρές κατακτήσεις" title="Οι πρώτες σου ανακαλύψεις">
              <Field label="Το πρώτο σου δοντάκι (και το πρώτο μου ξενύχτι)">
                <textarea style={textarea} value={values.firstTooth} onChange={onChange("firstTooth")} />
              </Field>
              <Field label="Λάτρευες να σε νανουρίζω έτσι">
                <textarea style={textarea} value={values.sleepRitual} onChange={onChange("sleepRitual")} />
              </Field>
              <Field label="Η πρώτη φορά που μπουσούλησες">
                <textarea style={textarea} value={values.crawlingFirstTime} onChange={onChange("crawlingFirstTime")} />
              </Field>
              <UploadBox
          ...
          {bookPage === 3 && (
            <Section emoji="" badge="Μεγαλώνεις" title="Τα πρώτα σου βήματα">
              <Field label="Η πρώτη φορά που σηκώθηκες όρθιο">
                <textarea style={textarea} value={values.stoodUpFirstTime} onChange={onChange("stoodUpFirstTime")} />
              </Field>
              <Field label="Η πρώτη σου τούμπα">
                <textarea style={textarea} value={values.firstFall} onChange={onChange("firstFall")} />
              </Field>
              <Field label="Η πρώτη φορά που έτρεξες προς το μέρος μου">
                <textarea style={textarea} value={values.ranToMe} onChange={onChange("ranToMe")} />
              </Field>
              <UploadBox
          ...
          {bookPage === 4 && (
            <Section emoji="️" badge="Αληθινές μέρες" title="Οι μέρες που δεν ήταν εύκολες">
              <Field label="Μια μέρα που ένιωσα ότι δεν τα κατάφερνα">
                <textarea style={textarea} value={values.hardDay} onChange={onChange("hardDay")} />
              </Field>
              <Field label="Μια στιγμή που λύγισα (αλλά δεν το έδειξα)">
                <textarea style={textarea} value={values.silentBreak} onChange={onChange("silentBreak")} />
              </Field>
              <Field label="Κάτι που με δυσκόλεψε περισσότερο απ’ όσο περίμενα">
                <textarea style={textarea} value={values.harderThanExpected} onChange={onChange("harderThanExpected")} />
              </Field>
              <Field label="Και παρ’ όλα αυτά... συνέχισα γιατί">
                <textarea style={textarea} value={values.iContinuedBecause} onChange={onChange("iContinuedBecause")} />
              </Field>
            </Section>
          )}
          ...
          {bookPage === 5 && (
            <Section emoji="" badge="Στιγμές καρδιάς" title="Οι στιγμές που με έκανες να νιώθω τα πάντα">
              <Field label="Η στιγμή που ένιωσα περήφανη για σένα">
                <textarea style={textarea} value={values.proudMoment} onChange={onChange("proudMoment")} />
              </Field>
              <Field label="Κάτι μικρό που για μένα ήταν τεράστιο">
                <textarea style={textarea} value={values.smallButHuge} onChange={onChange("smallButHuge")} />
              </Field>
              <Field label="Μια αγκαλιά που δεν ήθελα να τελειώσει">
                <textarea style={textarea} value={values.endlessHug} onChange={onChange("endlessHug")} />
              </Field>
              <Field label='Η στιγμή που σκέφτηκα "αυτό είναι η ευτυχία"'>
                <textarea style={textarea} value={values.thisIsHappiness} onChange={onChange("thisIsHappiness")} />
              </Field>
              <UploadBox
          ...
          {bookPage === 6 && (
            <Section emoji="" badge="Γιορτή" title="Tα γενέθλιά σου">
              <Field label="Σήμερα γίνεσαι ... ετών">
                <input style={input} value={values.birthdayAge} onChange={onChange("birthdayAge")} />
              </Field>
              <Field label="Έσβησες την τούρτα σου με αυτούς τους αγαπημένους σου">
                <textarea style={textarea} value={values.birthdayCakeWith} onChange={onChange("birthdayCakeWith")} />
              </Field>
              <Field label="Κάτι που έμαθες">
                <textarea style={textarea} value={values.birthdayLearned} onChange={onChange("birthdayLearned")} />
              </Field>
              <Field label="Μία στιγμή που δεν θα ξεχάσω από αυτή τη χρονιά">
                <textarea style={textarea} value={values.birthdayUnforgettableMoment} onChange={onChange("birthdayUnforgettableMoment")} />
              </Field>
              <Field label="Η ευχή μου για σένα">
                <textarea style={textarea} value={values.birthdayWish} onChange={onChange("birthdayWish")} />
              </Field>
              <UploadBox
          ...
          {bookPage === 7 && (
            <Section emoji="" badge="Έξω στον κόσμο" title="Τα πρώτα σου βήματα προς τον κόσμο">
              <Field label="Η πρώτη φορά που έπαιξες με άλλα παιδάκια">
                <textarea style={textarea} value={values.firstPlayWithKids} onChange={onChange("firstPlayWithKids")} />
              </Field>
              <Field label='Η πρώτη σου "φιλία" όπως την είδα εγώ'>
                <textarea style={textarea} value={values.firstFriendship} onChange={onChange("firstFriendship")} />
              </Field>
              <Field label="Κάτι που σε ενθουσίασε πολύ">
                <textarea style={textarea} value={values.excitement} onChange={onChange("excitement")} />
              </Field>
              <Field label="Κάτι που σε φόβισε">
                <textarea style={textarea} value={values.fear} onChange={onChange("fear")} />
              </Field>
              <Field label="Η στιγμή που κατάλαβα ότι μεγαλώνεις">
                <textarea style={textarea} value={values.iRealizedYouAreGrowing} onChange={onChange("iRealizedYouAreGrowing")} />
              </Field>
              <UploadBox
          ...
          {bookPage === 8 && (
            <Section emoji="" badge="Μεγάλο βήμα" title="Η πρώτη σου μέρα στο σχολείο">
              <Field label="Σήμερα πήγες για πρώτη φορά σχολείο...">
                <textarea style={textarea} value={values.firstSchoolDay} onChange={onChange("firstSchoolDay")} />
              </Field>
              <Field label="Και εγώ ένιωσα">
                <textarea style={textarea} value={values.schoolHowIFelt} onChange={onChange("schoolHowIFelt")} />
              </Field>
              <Field label="Εσύ έδειχνες">
                <textarea style={textarea} value={values.schoolHowYouLooked} onChange={onChange("schoolHowYouLooked")} />
              </Field>
              <Field label="Η στιγμή που σε άφησα">
                <textarea style={textarea} value={values.schoolWhenILeftYou} onChange={onChange("schoolWhenILeftYou")} />
              </Field>
              <Field label="Η σκέψη που δεν έφυγε από το μυαλό μου">
                <textarea style={textarea} value={values.schoolThoughtInMind} onChange={onChange("schoolThoughtInMind")} />
              </Field>
              <Field label="Όταν σε ξαναείδα">
                <textarea style={textarea} value={values.schoolWhenISawYouAgain} onChange={onChange("schoolWhenISawYouAgain")} />
              </Field>
              <UploadBox
          ...
          <div style={bookNav}>
            <button type="button" style={navButton} onClick={() => goToBookPage(bookPage - 1)} disabled={bookPage === 0}>
              Προηγούμενη
            </button>
            <button type="button" style={navButton} onClick={() => goToBookPage(bookPage + 1)} disabled={bookPage === 13}>
              Επόμενη
            </button>
          </div>
        </div>
...
function BookLogo({ onClick }: { onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} style={logoButton}>
...
function UploadBox({
  ...
  return (
    <div style={uploadCard}>
      ...
      <input type="file" accept="image/*,video/*" multiple={multiple} onChange={onChange} style={input} />
    </div>
  );
}
...
const book: CSSProperties = {
  maxWidth: "560px",
  margin: "0 auto",
  backgroundColor: "#B9907A",
  borderRadius: "28px",
  padding: "12px",
  boxShadow: "0 18px 40px rgba(80,50,35,0.22)",
};

const bookPaper: CSSProperties = {
  backgroundColor: "#FFFDF8",
  borderRadius: "22px",
  padding: "18px",
  minHeight: "620px",
  transformOrigin: "left center",
  boxShadow: "inset 10px 0 18px rgba(120,80,50,0.08)",
};
